...
try {
    const existing = firebase.apps.find(app => app.name === "GameHub");
    ...
    try {
        ...
        const result = await roomRef.transaction(roomData => {
            ...
            if (currentPlayers[user.uid]) {
                joinedColor = currentPlayers[user.uid].color || null;
                return roomData;
            }
            ...
            const used = new Set(
                Object.values(currentPlayers).map(player => player.color)
            );
        ...
        if (!result.committed && !joinedColor) {
            const latest = (await roomRef.once("value")).val();
            if (!latest) throw new Error("Phòng không còn tồn tại.");
            if (latest.status !== "waiting") throw new Error("Trận đấu đã bắt đầu.");
        ...
        if (!joinedColor) {
            const joinedPlayer = result.snapshot.val()?.players?.[user.uid];
            joinedColor = joinedPlayer?.color || null;
        }

        if (!joinedColor) {
            throw new Error("Không lấy được màu người chơi.");
        }
